...
const CategoryUpdate = () => {
  ...
  const { form, onSubmit } = useCategoryMutation({
    ...
    useEffect(() => {
      ...
      data && form.reset(data.category);
  }, [id, form.reset, data]);
  if (isLoading) return <div>Loading...</div>;
  return (
    <>
     <div className="my-20 sm:mx-auto sm:w-full sm:max-w-md">
  <h2 className="mt-6 text-center text-[25px] font-extrabold text-gray-900">Update Category</h2>
  <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
    <form onSubmit={form.handleSubmit(onSubmit)}  className="space-y-6" id="add-form">
      <label htmlFor='productName' className="block  text-[15px] font-medium text-gray-700">Category NAME</label>
